...
class VideoEventDetector:
    """A class for detecting events in videos using Google Gemini multimodal AI.

    This class handles extracting frames from videos, converting them to base64 format,
    and sending them to Google's Gemini API for event detection analysis.
    """
    ...
    def video_to_frames(
        ...
        try:
            ...
            for frame_idx in frame_indices:
                ...
                if success:
                    pil_image = self._convert_to_pil_image(frame)
                    if pil_image:
    ...
    def _convert_to_pil_image(self, frame: np.ndarray) -> Optional[Image.Image]:
        """Convert an OpenCV frame to PIL Image format.

        Args:
            frame: OpenCV image frame as numpy array (BGR format).
        ...
        try:
            ...
            logger.error(f"OpenCV error converting frame: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error converting frame: {e}")
            return None

    def _build_prompt(
        self, frames: List[Image.Image], events: List[Dict[str, str]], context: str
    ) -> tuple[str, List[Image.Image]]:
        """Create the prompt for Gemini API with images.
        ...
        Returns:
            Tuple of (prompt_text, images_list) for Gemini API.
        """
        events_list = "\n".join(
        ...
        system_prompt_text = self.system_prompt.format(context=context)
        user_prompt_text = self.user_prompt.format(events_list=events_list)

        # Combine system and user prompts with clear formatting for Gemini
        full_prompt = f"{system_prompt_text}\n\n{user_prompt_text}\n\n"
        full_prompt += "Please respond with a JSON object containing an 'events' array. Each event should have 'event_code', 'detected' (boolean), and 'explanation' fields."

        return full_prompt, frames
    ...
    def detect_events(
        ...
        logger.info(
            f"Extracted {len(frames)} frames from {video_path}. Sending to Gemini."
        )
        ...
        try:
            ...
            results_content = response.text.strip()

            # Remove markdown code fences if present (```json ... ```)
            if results_content.startswith("```"):
                # Find first newline after opening fence
                first_newline = results_content.find("\n")
                if first_newline != -1:
                    # Remove opening fence (e.g., ```json or ```)
                    results_content = results_content[first_newline + 1 :]
                # Remove closing fence
                results_content = results_content.rsplit("```", 1)[0]
                results_content = results_content.strip()

            # Try to parse JSON from the response
            results = json.loads(results_content)
            logger.info(f"Received Gemini response for {video_path}")

        except json.JSONDecodeError as e:
            logger.error(
                f"Failed to decode JSON response from Gemini for {video_path}: {e}. Response content: {results_content}"
            )
            return {"error": "Failed to decode Gemini response"}
        except Exception as e:
            logger.error(f"Error calling Gemini API for {video_path}: {e}")
        ...
        if self.output_queue is not None:
            try:
                ...
                if "events" in results:
                    ...
                    logger.warning(
                        f"Gemini response for {video_path} did not contain 'events' key."
                    )
            except Exception as e:
                logger.error(
                    f"Error processing Gemini results or queuing event alerts for {video_path}: {e}"
                )
