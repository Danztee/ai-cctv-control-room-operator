            ...
            try:
                if cap is None or not cap.isOpened():
                    ...
                    if not cap.isOpened():
                        ...
                        logger.warning(
                            f"Failed to open stream {self.stream_url}. Retrying in {retry_delay} seconds... ({consecutive_failures}/{max_consecutive_failures} failures)"
                        )
                        
                        if consecutive_failures >= max_consecutive_failures:
                            logger.error(f"Failed to connect to stream after {max_consecutive_failures} attempts. Stopping chunker.")
                            break
